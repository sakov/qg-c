...
!
! Purpose:        Solver for Helmholtz equation.
!
! Description:    This file has been derived from the file with the same name
!                 in the QG model code from enkf-matlab package, where it came
!                 from http://www.seas.harvard.edu/climate/eli/Downloads/QG200205011455.tar.gz.
!                 The actual author unknown. I attempted to re-write it in C,
!                 but quickly resigned after realising the effort required.
!
! Revisions:
...
contains
  ...
  subroutine helmholtz(G, F, zk2, NX1, NY1, M, H1, NU1, NU2, NCYC, NX, NY, P)
    !
    ...
    ! initial guess.                                                  **
    ! The dimension of the work array q should be NX * NY * 3.          **
    !
    real(8), intent(in), dimension(NX, NY) :: G, F
    real(8), intent(in) :: zk2
    integer, intent(in) :: NX1, NY1, M
    real(8), intent(in) :: H1
    integer, intent(in) :: NU1, NU2, NCYC, NX, NY
    real(8), intent(out), dimension(NX, NY) :: P
 ...
 subroutine relax(k, krhs, wu, M, zk2)
   ...
   integer, dimension(NXY) :: ist, irhs
   integer :: ii, jj
   real(8) :: hh
 ...
 subroutine intadd(kc, kf)
   ...
   integer, dimension(NXY) :: istc, istf
   real(8) :: hc, hf
 ...
 subroutine rescal(kf, krf, krc, zk2)
   ...
   integer, dimension(NXY) :: iuf, irf, irc
